...
Computes column-level and grouped descriptive statistics using Polars.
Accepts a CSV file path as a command-line argument so the same script
works on any well-formed CSV.
...
# ─── Configuration ───────────────────────────────────────────────────
MISSING_TOKENS = {"", "na", "n/a", "null", "none", "nan", "NA", "N/A"}
MAX_TOP_VALUES = 5
...
def normalize_missing(df):
    ...
    for col in df.columns:
        if df[col].dtype == pl.Utf8 or df[col].dtype == pl.String:
            df = df.with_columns(
                pl.when(pl.col(col).str.strip_chars().str.to_lowercase().is_in(list(MISSING_TOKENS)))
                .then(None)
...
def infer_column_type(series):
    """Classify a column as numeric, date, identifier, or categorical."""
    ...
    date_attempt = sample.str.to_date("%Y-%m-%d", strict=False)
    ...
    for col in df.columns:
        ...
        if col_type == "numeric":
            if series.dtype in (pl.Float32, pl.Float64, pl.Int8, pl.Int16,
                ...
                nums = series.cast(pl.Utf8).str.strip_chars().cast(pl.Float64, strict=False).drop_nulls()
    ...
    for idx in range(min(MAX_GROUPS_PRINTED, group_sizes.height)):
        ...
        for c in numeric_agg:
            if group_df[c].dtype in (pl.Float32, pl.Float64, pl.Int8, pl.Int16,
                ...
                nums = group_df[c].cast(pl.Utf8).str.strip_chars().cast(pl.Float64, strict=False).drop_nulls()
